...
const [expansionCoefficient, setExpansionCoefficient] =
useState("0.000012");
...
const pipeID =
OD > 0 && T > 0 && OD > 2 _ T ? OD - 2 _ T : 0;

const outsideArea =
OD > 0 ? (Math.PI _ OD _ OD) / 4 : 0;

const insideArea =
pipeID > 0 ? (Math.PI _ pipeID _ pipeID) / 4 : 0;
...
const steelArea =
OD > 0 && pipeID > 0
? (Math.PI / 4) _ (OD _ OD - pipeID \* pipeID)
: 0;

const pipeVolume =
pipeID > 0 && L > 0
? (Math.PI / 4) _ (pipeID / 1000) \*\* 2 _ L
: 0;
...
const massPerMeter =
steelArea > 0
? (steelArea / 1_000_000) \* RHO
: 0;

const totalMass =
massPerMeter > 0 && L > 0
? massPerMeter _ L _ QTY
: 0;
...
MU > 0 &&
RHO > 0
? (RHO _ selectedVelocity _ (pipeID / 1000)) / MU
: 0;
...
? frictionFactor _
(RUN / (pipeID / 1000)) _
(RHO _ selectedVelocity _ selectedVelocity) /
2
: 0;

const pressureDropBar =
pressureDropPa > 0
? pressureDropPa / 100000
: 0;
...
const thicknessWithCA =
pressureThickness > 0
? pressureThickness + CA
: 0;
...
const rollingOffset =
A > 0 && B > 0
? Math.sqrt(A _ A + B _ B)
: 0;

const offsetAngle =
A > 0 && B > 0
? (Math.atan(B / A) \* 180) / Math.PI
: 0;
        ...
        {notice && (
          ...
          >
            ...
            <p style={{ color: "#94a3b8" }}>
              Always verify final engineering design against the
              applicable project code, specification and approved
        ...
        {activeTab === "dashboard" && (
          <>
            ...
            >
              ...
              >
                This portal provides preliminary engineering calculations
                and quick-reference tools. Final design, code compliance,
                material selection, pressure rating, stress analysis and
                fabrication decisions must be checked against the applicable
                project specification, approved drawings and governing code.
              </p>
            </div>
          </>
        )}
            ...
            >
              ...
              {calcCard({
                ...
                formula:
                  "ID = OD − 2t",
                note:
                  "Use consistent dimensions. This is a geometric calculation.",
                children: (
                  <>
                    <div style={resultStyle}>
                      ID: {pipeID.toFixed(2)} mm
                    </div>
              ...
              {calcCard({
                ...
                formula:
                  "Mass/m = Steel Area × Density",
                note:
                  "Actual supplied weight may differ because of manufacturing tolerances and coating/lining.",
                children: (
                  <>
                    <div style={resultStyle}>
              ...
              {calcCard({
                ...
                description:
                  "Approximate internal volume for a straight pipe.",
                formula:
                  "V = π/4 × ID² × L",
                note:
                  "Calculated using the internal diameter and straight pipe length.",
                children: (
                  <div style={resultStyle}>
              ...
              {calcCard({
                ...
                formula:
                  "v = Q / A",
                note:
                  "Flow must be volumetric flow rate. Confirm operating conditions and units.",
                children: (
                  <>
                    <input
                      type="number"
              ...
              {calcCard({
                ...
                formula:
                  "Re = ρvD / μ",
                note:
                  "For this calculator, density is kg/m³, velocity m/s, diameter m and viscosity Pa·s.",
                children: (
                  <>
                    <input
                      type="number"
              ...
              {calcCard({
                ...
                description:
                  "Estimate straight-pipe frictional pressure loss.",
                formula:
                  "ΔP = f × (L/D) × ρv²/2",
                note:
                  "This calculates straight-pipe friction only. Fittings, valves, elevation and other losses are not included.",
                children: (
                  <>
                    <input
                      type="number"
              ...
              {calcCard({
                ...
                formula:
                  "Ptest = 1.5 × Pdesign",
                note:
                  "Preliminary reference only. Actual test pressure must be established from the governing code, project specification, component limits and applicable temperature requirements.",
                children: (
                  <>
                    <input
                      type="number"
                      ...
                      onChange={(e) =>
                        setDesignPressure(e.target.value)
                      }
                      style={inputStyle}
              ...
              {calcCard({
                ...
                formula:
                  "t = P × D / [2(SE + PY)]",
                note:
                  "This is not a complete piping code design check. Material, temperature, weld factor, corrosion, mechanical loads, tolerances and code requirements must be verified separately.",
                children: (
                  <>
                    <input
                      type="number"
                      ...
                      onChange={(e) =>
                        setAllowableStress(e.target.value)
                      }
                      style={inputStyle}
              ...
              {calcCard({
                title: "⭕ Hoop Stress",
                description:
                  "Thin-wall hoop-stress reference calculation.",
                formula:
                  "σh ≈ P × D / (2t)",
                note:
                  "Use consistent pressure and dimensional units. This is a simplified stress calculation, not a complete code stress analysis.",
                children: (
                  <>
                    <input
                      type="number"
              ...
              {calcCard({
                ...
                formula:
                  "ΔL = α × L × ΔT",
                note:
                  "Coefficient must be in 1/°C or 1/K and length must use the desired output unit.",
                children: (
                  <>
                    <input
                      type="number"
              ...
              {calcCard({
                ...
                formula:
                  "Travel = √(A² + B²)",
                note:
                  "Use the same unit for A and B. This is basic right-triangle geometry.",
                children: (
                  <>
                    <input
                      type="number"
          ...
          <>
            ...
            >
              ...
              >
                ...
                <option value="bar-psi">
                  bar → psi
                </option>

                <option value="psi-bar">
                  psi → bar
                </option>

                <option value="m3h-lpm">
                  m³/h → L/min
                </option>

                <option value="lpm-m3h">
                  L/min → m³/h
                </option>
